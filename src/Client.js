...
module.exports = class Client extends EventEmitter {
    ...
    sendMessage (channelId, data) {
        return new Promise((resolve, reject) => {
            if (!this.token) { reject(new Error('No token provided, use Client#login() to set it')) }
            if (!channelId) { reject(new Error('No channelId provided, channelId is required to use Client#sendMessage()')) }
            const form = new FormData()
            if (data.file && typeof data.file === 'object') { form.append(data.file.name, data.file.attachment, data.file.name) }
            if (data.files) {
            ...
            if (data.embed && data.embed.rich) { data.embed = data.embed.rich }
            if (data.embeds) {
                for (let i = 0; i < data.embeds.length; i++) {
                    const embed = data.embeds[i]
                    if (embed && embed.rich) { data.embed[i] = embed.rich }
                }
            }
